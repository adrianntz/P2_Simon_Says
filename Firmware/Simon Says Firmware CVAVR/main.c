...
int Sequence_Input[MAX_LEVEL];
int level = 1;
int Sequence_Step_Delay = 1000;
...
void right_Current_Level() {
  ...
  Sequence_Step_Delay -= 50; //increases difficulty
}
...
void wrong_Current_Level() 
{
  ...
  level = 1;
  Sequence_Step_Delay = 1000;
}

void get_Current_Level() 
{
  int flag = 0; //flag correct Current_Level
  unsigned int i;
  ...
  for (i = 0; i < level; i++) {
    ...
    while (flag == 0) 
    {
      ...
      if (!SW1) 
      {
        ...
        if (Sequence_Input[i] != Current_Level[i]) {
          wrong_Current_Level();
          return;
        }
        LED1=0;
